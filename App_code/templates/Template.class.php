...
class Template {
	...
	private function subst($varname)
		...
		//$method = str_replace( " ", "", ucwords( str_replace( "_", " ", strtolower( $p ) ) ) ) ;
		...
		elseif( method_exists( $inst, "__get" ) )
                {
                    $s = str_replace( "{" . $varname . '->' . $p . "}", $inst->__get( $p ), $s ) ;
                }
                else
                {
                    throw new BadMethodCallException( "no accessor method in class " . get_class( $inst ) . " for " . $varname . "->" . $p ) ;
                }
            }
            return $s ;
	}
	 ...
	 * content.
	 *
	 * If the parameter $append is true, the block 
	 * content will be appended with the previous content.
	 *
